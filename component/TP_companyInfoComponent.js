async function executeTP_CompanyInfo(data){
    let basicInfoData = []
    let financeInfo = []
    let dividendRecord = []
    let closingPrice = []
    ...
    async  function handleAllData(){
        const fetchedCompanyInfoData = await getCompanyInfo(data)
        if (fetchedCompanyInfoData.status === true){
            ...
            if(closingPrice && priceData && basicInfoData){
                renderClosingPrice(data)
            }
        }
        const fetched365ClosingPriceData = await getGraphdata(data, 365)
